...
class SCGODataConnection:
    ...
    def __getattr__(self, name):
        """Delegate unknown attributes to the wrapped ``DataConnection``."""
        return getattr(self._da, name)

    def __enter__(self):
        """Enter the context manager and return this wrapper."""
        return self

    def __exit__(self, exc_type, exc, tb):
        """Close the wrapped ``DataConnection`` on context exit (best effort)."""
    ...
    def add_relaxed_step(self, a, *args, **kwargs):
        """Validate stoichiometry, ensure ``raw_score``/``final_id``, then insert.

        Args:
            a: Relaxed candidate to persist.
            *args: Forwarded to ``DataConnection.add_relaxed_step``.
            **kwargs: Forwarded to ``DataConnection.add_relaxed_step``.

        Returns:
            Whatever the wrapped ``DataConnection.add_relaxed_step`` returns.

        Raises:
            SCGOValidationError: If the candidate composition does not match the
                database stoichiometry.
        """
    ...
    def add_unrelaxed_candidate(self, a, *args, **kwargs):
        """Insert an unrelaxed candidate with the ASE-required info bags present.

        Args:
            a: Unrelaxed candidate to persist.
            *args: Forwarded to ``DataConnection.add_unrelaxed_candidate``.
            **kwargs: Forwarded to ``DataConnection.add_unrelaxed_candidate``.

        Returns:
            Whatever the wrapped ``DataConnection.add_unrelaxed_candidate`` returns.
        """
...
def _ensure_database_indices(
    ...
    *,
    ...
    try:
        ...
        logger.warning("Unexpected error creating indices on %s: %s", db_path, e)
...
def _extract_structures_from_db(  # noqa: C901
    db_path: str | Path,
    run_id: str,
    *,
...
def load_previous_run_results(  # noqa: C901
    base_output_dir: str,
    ...
    if use_parallel:
        ...
        try:
            with ProcessPoolExecutor(max_workers=resolved_max_workers) as executor:
                ...
                for future in as_completed(futures):
                    db_path, _run_id = futures[future]
                    try:
        ...
        logger.info("Loading %d databases sequentially", len(all_db_files))

        for db_path, entry_run_id in all_db_files:
            minima = extract_minima_from_database_file(
                db_path, entry_run_id or "", require_final=prefer_final_unique
            )
...
def load_reference_structures(  # noqa: C901
    db_glob_pattern: str,
    composition: list[str] | None = None,
...
def _load_single_database_worker(
    ...
    try:
        ...
        logger.error("Failed to extract minima from %s in worker: %s", db_path, e)
        return []
