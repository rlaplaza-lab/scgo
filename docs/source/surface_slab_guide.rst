Surface slabs for nanocluster global optimization
=================================================

This guide shows how to build graphene and graphite slabs and use them as
substrates for nanocluster global optimization (GO) — both pristine and with
defects (vacancies, dopants). It complements the API reference in
:doc:`/api/surface`, and the quick-start recipes in :doc:`/quickstart`.

SCGO supports two slab families for ``surface_cluster`` / ``surface_cluster_adsorbate``
(deposit and optimize a nanoparticle on top) and for ``surface`` /
``surface_adsorbate`` (let GA/BH move the top slab layers, with a deposited
fragment, and leave the lower layers fixed).

.. note::

   All slabs use **slab-style PBC**: periodic in the two in-plane directions and
   **open along the vacuum (surface-normal) axis**. For the default orientation
   the surface normal is ``z`` (``pbc=(True, True, False)``). This matches
   :class:`~scgo.surface.config.SurfaceSystemConfig`, which normalizes any slab
   you pass and raises if no periodic dimension is present.
...
.. list-table::
   ...
   * - Slab
     - Use when
   * - Graphite (pristine)
     - Multi-layer support; the deposited cluster sits on the top graphene sheet.
       Built with :func:`~scgo.make_graphite_surface_config`.
   * - Graphene (pristine, monolayer)
     - Single free-standing sheet (Chepkasov-style support). Built with
   ...
   * - Monovacancy graphene
     - Single-layer graphene with one removed carbon atom; the deposition
       pipeline can target the vacancy. Built with
       :func:`~scgo.make_graphene_surface_config` (``monovacancy=True``).
   * - N-doped graphite
...
The vacancies are removed from the **top layer** of the graphite stack. To use
the same slab directly as the GO target (no nanoparticle core), pass an empty
...
.. code-block:: python
   ...
   # Optional Jahn-Teller seed: displace two nearest vacancy neighbours
...
bond axes, seeding the characteristic reconstruction around the vacancy.
...
``defect_bias_probability > 0`` on the config, the deposition pipeline
redirects a fraction of placements onto the vacancy, while still falling back to
random placement the rest of the time. This guarantees non-zero defect coverage
...
.. code-block:: python
   ...
   # Land on the vacancy every time (deterministic targeting)
   cfg = make_graphene_surface_config(
...
The bias only overrides the **in-plane** center of the cluster; the height above
the slab follows the normal adsorption-height heuristics, so the cluster still
sits in the bonding range of the vacancy's three neighbours. Setting
...
Add an adsorbate (e.g. ``OH``) and switch to ``surface_adsorbate`` to optimize
the top layers plus the fragment; see :doc:`/quickstart` for the full
adsorbate recipe.

Tuning deposition
-----------------

Key knobs on :class:`~scgo.surface.config.SurfaceSystemConfig`:
...
* ``adsorption_height_min`` / ``adsorption_height_max`` — vertical gap between
  ...
  0.5–1.5 Å; graphite and defected-graphite presets use 0.5–1.0 Å, so the
  cluster stays bonded to the surface (or to the three vacancy neighbours).

  This tight band is a *deliberate* chemisorption-contact bias: it seeds the
  mobile cluster at chemical-bonding distances rather than out in the
  physisorption tail, trading away physisorption sampling (and biasing toward
  hollow / vacancy sites) for a higher bonding hit-rate. It is **not** a free
  standoff — every accepted placement is still gated by the ``blmin``
  two-set clash check (``blmin_ratio = 0.7``, i.e. Pt–C ≥ 1.48 Å), so the window
  can be widened or overridden per run via these two knobs without relaxing the
  steric guarantee.
