...
def assert_ts_result_valid(
    ...
    *,
    ...
    if require_interior_ts:
        ...
        # Images are [reactant] + n_images interiors + [product]; endpoints sit at
        # index 0 and n_images + 1, so the valid interior range is 1..n_images
        # (inclusive). A saddle on the last interior image (ts_idx == n_images) is
        # legitimate and must not be rejected.
        assert 0 < int(ts_idx) < int(n_images) + 1, (
            f"TS image index {ts_idx} not interior for n_images={n_images} "
            f"(valid range 1..{n_images})"
        )
...
def assert_deposition_height_in_bounds(
    ...
    *,
    ...
    ``SurfaceSystemConfig.adsorption_height_min/max`` constrain only the
    **bottom** of the deposited cluster: ``scgo.surface.deposition`` places the
    cluster bottom at ``slab_top + effective_height`` where
    ``effective_height = max(h_min, min(sampled, target_height))`` and
    ``sampled ~ Uniform(h_min, h_max)``. So only the *bottom gap*
    ``min(mobile_z) - slab_top`` is guaranteed inside ``[h_min, h_max]``; a
    non-spherical cluster's top atoms legitimately sit far outside. GA and NEB
    may move atoms further during relaxation -- use
    :func:`assert_supported_cluster_binding` after relaxation instead.
    """
    ...
    bottom_gap = float(np.min(mobile_positions[:, axis]) - slab_top)
    assert h_min - tolerance <= bottom_gap <= h_max + tolerance, (
        f"Deposition bottom gap {bottom_gap:.3f} Å not in [{h_min}, {h_max}] "
        f"± {tolerance}"
    )
    # No mobile atom may be buried below the slab top (the sampler never does
    # this; a violation means a placement/relaxation regression).
    assert bottom_gap >= -1e-6, (
        f"Bottom-most mobile atom is {abs(bottom_gap):.3f} Å below slab top (buried)"
    )
...
def assert_minima_structurally_valid(
    minima: list[tuple[float, Atoms]],
    *,
    expected_n_atoms: int,
) -> None:
    """Per-entry structural validation for a ``(energy, Atoms)`` minima list.

    Catches "easy pass" bars that only check ``isinstance(list)`` / ``len >= 1``
    by asserting every entry is a finite, correctly sized, finite-positioned
    structure with a finite energy.
    """
    assert minima, "Expected at least one minimum"
    for energy, atoms in minima:
        assert np.isfinite(energy), "Minimum energy must be finite"
        assert isinstance(atoms, Atoms), "Minimum must be an Atoms object"
        assert len(atoms) == expected_n_atoms, (
            f"Expected {expected_n_atoms} atoms, got {len(atoms)}"
        )
        assert np.all(np.isfinite(atoms.get_positions())), (
            "Minimum positions must be finite"
        )
...
def assert_e2e_go_ts_summary(
    ...
    *,
    ...
    example matrix for surface systems. It refuses any OOM / never-ran band (the
    silent-swap regression guard) and, when ``require_ts_success=True`` (the
    default), additionally demands at least one *successful* saddle (or a
    fully-converged *barrierless* reaction whose every band peaks at an
    endpoint). ``require_ts_success=False`` relaxes only the "must find a
    saddle" demand for system types that are genuinely stochastic under the
    low-effort CI budget (e.g. heavy adsorbate/surface cases on a 16 GB T4),
    while the degradation guard stays strict so a dropped band still fails. Cases
    whose CI budget legitimately leaves no on-disk pairs (gas examples) keep
    ``require_ts_candidates=False`` and only get the structural checks below.
    """
    ...
    if require_ts_candidates:
        ...
        # was dropped". Reject any OOM / never-ran band so a silent GPU
        # regression cannot ship a green run, independent of require_ts_success.
        degraded = [
        ...
        if require_ts_success:
            assert summary["ts_total_count"] >= 1
            assert ts_results, "Expected TS result dicts"
            # Demand at least one interior saddle, OR a fully-converged
            # *barrierless* reaction (every band peaks at an endpoint, i.e. the
            # product is the local maximum and no interior saddle exists).
            # MACE-MP in particular yields monotonic-to-product paths for some
            # adsorbate systems; that is legitimate chemistry, not a
            # silently-dropped band, so it must not fail the trial of fire. The
            # degradation guard above still rejects OOM / never-ran bands.
            if not summary["ts_success_count"] >= 1:
                barrierless = all(
                    ts_band_is_barrierless_endpoint(r) for r in ts_results
                )
                assert barrierless, (
                    "Expected at least one successful transition state, got "
                    f"{summary['ts_success_count']} of "
                    f"{summary['ts_total_count']} pairs; "
...
def validate_structure_with_diagnostics(
    ...
    is_valid, error_msg = validate_cluster_structure(
        ...
        connectivity_factor,
        check_clashes=True,
        check_connectivity=_should_check_connectivity(atoms, uses_surface=False),
    )
...
def _make_minima_atoms(raw_score: float, run_id: str) -> Atoms:
    """Create a Pt atom with the metadata required by ``filter_unique_minima``."""
    atoms = Atoms("Pt", positions=[[0, 0, 0]])
    atoms.info = {"key_value_pairs": {"raw_score": raw_score, "run_id": run_id}}
    return atoms


def _pt3_with_calc() -> tuple[Atoms, Any]:
    """Return a Pt3 cluster with an attached EMT calculator."""
    atoms = Atoms(
        "Pt3",
