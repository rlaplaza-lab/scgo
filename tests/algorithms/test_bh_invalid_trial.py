...
def _patch_validation_failure(
    ...
    """Make ``validate_structure_for_system_type`` raise once, on ``fail_on_call``."""
    calls = {"n": 0}
    real = basinhopping_go.validate_structure_for_system_type
    ...
    monkeypatch.setattr(basinhopping_go, "validate_structure_for_system_type", _flaky)
    return calls
...
def test_bh_run_completes_when_last_trial_is_invalid(tmp_path, monkeypatch) -> None:
