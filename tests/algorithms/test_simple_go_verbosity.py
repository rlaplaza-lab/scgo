...
def test_simple_go_verbosity_zero_emits_no_info_logs(tmp_path, caplog, rng) -> None:
...
def test_simple_go_verbosity_one_emits_info_logs(tmp_path, caplog, rng) -> None:
    with caplog.at_level(logging.DEBUG, logger=_SIMPLE_GO_LOGGER):
        simple_go(
            _pt2_with_calc(),
            str(tmp_path / "normal"),
