...
Example-mimic GPU integration coverage (MACE): `tests/integration/test_gpu_examples_integration.py` (all six `system_type` values from `examples/`). Shared e2e bars require run-dir `metadata.json`, SCGO-stamped `*.db`, and (for cluster-bearing cases) at least one TS candidate.
