...
- **Global optimization (`params` for `run_go` / `run_go_campaign`)** is merged with `get_default_params()` via `initialize_params`: pass `None`, a partial dict, or a preset from `get_default_params()`, `get_minimal_ga_params()`, `get_testing_params()`, `get_high_energy_params()`, `get_diversity_params()`, `get_default_uma_params()`, or `get_torchsim_ga_params(system_type=..., surface_config=..., seed=..., model_name=...)`.
- **Transition-state search (`ts_params` for `run_ts_search` / `run_go_ts`)** is merged with `get_ts_search_params(...)` via `initialize_ts_params` (not with GO defaults). Pass `None`, a partial dict, or a full preset from `get_ts_search_params(...)`.
- At **`verbosity=1`**, runners log which preset defaults were used and which user keys overrode them before printing resolved GO/TS settings.
